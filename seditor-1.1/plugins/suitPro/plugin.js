...
SEDITOR.plugins.add('suitPro', {


    custom: {
        $dialog: $('<div class="widget-suitPro" style="display: none;"></div>'),
        basesrc:'',
        urls: {
            query: ''
        },
        content: {
            part1: {
                ...
                $search: '',
                album: '<div class="album"><div class="container"></div></div>',
                $album: ''
            },
            part2: {
                main: '<div></div>',
        ...
        create: function (widget) {
            ...
            this.content.part1.$album = $(this.content.part1.album);
            this.content.part1.$main.append(this.content.part1.$album);
        ...
        findTarget: function (widget) {
            var $imgs = widget.$element.find('[data-toggle-img="src"]'),$this;
            $($imgs).each(function(){
                if(SEDITOR.tools.widget.hasAttr(widget.$element,['widget-toggle','position-toggle'],$this = $(this))){
                    var basesrc = $this.attr('basesrc');
                    if (typeof basesrc === 'undefined') {
                        basesrc = '';
                    } else if (basesrc.lastIndexOf('/') !== (basesrc.length - 1)) {
                        basesrc += '/';
                    }
                    widget.custom.basesrc = basesrc;
                ...
                buttons: [
                    {
                        ...
                        click: function () {
                            var $product = widget.custom.content.part1.$album.find('.item.selected');
                            if ($product.length === 1) {
                                widget.custom.fillData(widget, $product.data('data'));
                            }
        ...
        search: function (widget, config) {
            ...
            //test
            /* $.getJSON("http://localhost:63342/seditor-1.0/plugins/suitPro/data.json", config,
             function (result) {
             if (result && result['status'] === 'success') {
             widget.custom.show(widget, result);
             } else {
        ...
        show: function (widget, result) {
            var $items, $item, item = '<div class="item"><img /><div class="name"></div></div>',
                $container = this.content.part1.$album.show().find('.container').empty();
            widget.custom.cache.kw = result['kw'];
            widget.custom.cache.page = parseInt(result['page']);
            widget.custom.cache.max = parseInt(result['max']);

            this.content.part1.$album.find('.page').remove();

            $(result['suitList']).each(function () {
                $item = $(item);
                if(this.img){
                    $item.find('img').attr('src', widget.custom.basesrc + this.img);
                }else{
                    $item.find('img').replaceWith('<div class="img-miss">暂无图片</div>');
                }
                $item.find('.name').text(this.name);
                $item.data('data', this);
        ...
        page: {
            ...
            create: function (widget, result) {
                this.$html = $(this.html);
                widget.custom.content.part1.$album.append(this.$html);
        ...
        fillData: function (widget, data) {
            var $dataElements, $dataElement, label, custom = this, position, map = this.map, $item, dataItem, $template;
            ...
            for (var fieldName in map) {
                ...
                $dataElements.each(function () {
                    ...
                    if (SEDITOR.tools.widget.hasAttr(widget.$element, ['position-toggle', 'widget-toggle'], $dataElement)) {
            ...
            //商品列表
            var $products = widget.$element.find('[position-toggle="products"]');
            $template = $products.find('[position-toggle="item"]').first();
            if ($products.length > 0 && $template.length > 0 && data['pros'].length >0) {

                //清理多余项
                $template.siblings('[position-toggle="item"]').remove();

                $(data['pros']).each(function (index) {
                    $item = $template.clone(true);
                    dataItem = this;
                    for (var fieldName in map) {
                        label = 'data-toggle-' + fieldName;
                        $dataElements = SEDITOR.tools.findElements($item,label);
                        $dataElements.each(function () {
                            $dataElement = widget.seditor.$(this);
                            if (SEDITOR.tools.widget.hasAttr($item, ['position-toggle', 'widget-toggle'], $dataElement)) {
                                position = $dataElement.attr(label);
                                custom.fillDataToPosition($dataElement, label, position, dataItem[map[fieldName]]);
                            }
                        });
                    }


                    if (data['pros'].length === (index + 1)) {
                        //连接符
                        var connector = $item.find('[other-toggle="connector"]');

                        //去掉最后一个子项连接符
                        if (connector.length > 0) connector.remove();
                    }
            ...
            //图片列表
            var $imgs = widget.$element.find('[position-toggle="imgs"]');
            $template = $imgs.find('[position-toggle="item"]').first();


            if ($imgs.length > 0 && $template.length > 0 && data['imgs'].length >0) {
                ...
                $(data['imgs']).each(function (index) {
                    $item = $template.clone(true);
                    dataItem = this;
                    label = 'data-toggle-img';
                    $dataElements = SEDITOR.tools.findElements($item,label);
                    $dataElements.each(function () {
                        $dataElement = widget.seditor.$(this);
                        if (SEDITOR.tools.widget.hasAttr($item, ['position-toggle', 'widget-toggle'], $dataElement)) {
                            position = $dataElement.attr(label);
                            custom.fillDataToPosition($dataElement, label, position, dataItem);
                        }
                    });

                    if (data['imgs'].length === (index + 1)) {
                        //连接符
                        var connector = $item.find('[other-toggle="connector"]');
            ...
            for (var fieldName in map) {
                ...
                $dataElements.each(function () {
                    $dataElement = $(this);
                    if (SEDITOR.tools.widget.hasAttr(widget.$copyElement, ['position-toggle', 'widget-toggle'], $dataElement)) {
                        position = $dataElement.attr(label);
                        custom.fillDataToPosition($dataElement, label, position, data[map[fieldName]]);
                    }
                });
            ...
            //商品列表
            var $products = widget.$copyElement.find('[position-toggle="products"]');
            $template = $products.find('[position-toggle="item"]').first();
            if ($products.length > 0 && $template.length > 0 && data['pros'].length >0) {
            ...
            //图片列表
            var $imgs = widget.$copyElement.find('[position-toggle="imgs"]');
            $template = $imgs.find('[position-toggle="item"]').first();
