...
SEDITOR.plugins.add('product', {

    custom: {
        $dialog: $('<div class="widget-product" style="display: none;"></div>'),
        basesrc: '',
        urls: {
            query: ''
        },
        ...
        search: function (widget, config) {
            ...
            //test
            /*$.getJSON("http://localhost:63342/seditor-1.0/plugins/product/data.json", config,
             function (result) {
             if (result && result['status'] === 'success') {
             widget.custom.show(widget, result);
             } else {
