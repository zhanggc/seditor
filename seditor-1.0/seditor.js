...
(function () {
    window.SEDITOR || (window.SEDITOR = new function () {
        ...
        this.SEditor = function (name, $dest, plugins) {
            ...
            this.getCopyElement = function () {
                ...
                $content[0].innerHTML = this.getConfig().content.data;
                return $content;
            }
            ...
            this.getData = function () {
                var $textarea;
                if (this.element != null && this.element.readyState) {
                    this.getConfig().content.data = this.element.$.html();
                } else if (($textarea = this.$dest.find('.sed_contents textarea')).length > 0) {
                    this.getConfig().content.data = $textarea.val();
                }

                //过滤
                this.filter();
                return this.getConfig().content.data;
            };
            ...
            this.parseScript = function () {
                ...
                $($scripts).each(function () {
                    var $this = $(this),
                        $marker = seditor.SEDITORUTILS.$('<div></div>').insertBefore(this),
    ...
    SEDITOR.plugins = {
        ...
        widget: function () {
            ...
            this.filter = function () {
                var $content = this.seditor.getCopyElement(), holders = $content.find('[' + this.marker + '="' + this.id + '"] .widget-holder');
                if (holders.length > 0) {
                    holders.remove();
                    this.seditor.getConfig().content.data = $content.html();
                }
